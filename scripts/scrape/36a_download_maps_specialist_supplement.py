"""Download only specialist Maps tasks explicitly listed by Tasks Ready."""
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check readiness or download the 75 Maps specialist tasks."
    )
    ...
    return resolve_run_context_arguments(
        args,
        {
            ...
            "output_directory": (
                "raw",
                "maps_specialist_supplement/raw",
            ),
        },
    )
...
def main() -> int:
    ...
    submitted = (
        log.loc[log["submission_status"].eq("submitted")]
        .drop_duplicates("task_tag", keep="last")
        .copy()
    )
    if len(submitted) != 75 or submitted["task_id"].isna().any():
        raise ValueError("Specialist task log requires all 75 submitted task IDs")
    ...
    for row in ready_rows.itertuples(index=False):
        payload = client.get_task(
            str(settings["endpoints"]["maps_get"]), str(row.task_id)
        )
        ...
        json.dumps(
            {
                ...
                "tasks_not_ready": len(pending) - len(ready_rows),
                ...
                "remaining_after_download": len(pending) - len(ready_rows),
            },
            indent=2,
        )
    )
