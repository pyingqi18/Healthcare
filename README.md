...
Stage `46a` is the active boundary. Its first pass combined 33,238 source rows into 29,984 exact Google profiles, removing 3,254 repeated source observations without merging physical locations. The first pending queue contained 583 blank decisions. Reusing 13 prior exact-profile decisions reduced it to 570, and 120 unanimous frozen category exclusions produced the canonical 450-profile manual-review cohort. The untouched baseline contains zero decisions and 410 final review blocks; it is frozen under `archive/profile_eligibility_review/original_manual_review_20260924`. Later files are progress checkpoints, not original inputs. Stage `46e` extracted 281 unresolved external-evidence records into 252 audit units. The first evidence batch checked every member of the 20 shared-domain units at its exact profile and address, producing 34 inclusions and 15 exclusions. The current exact-profile table therefore contains 212 decisions, comprising 121 inclusions and 91 exclusions, with 238 of the original 450 profiles still unresolved. Stage `46h` confirms that all 238 remaining records are singleton profiles and places 39 records with specific official service, location, or provider pages into the first audit batch; this routing is not an eligibility decision. The v8 checkpoint with 163 decisions remains under `archive/profile_eligibility_review/20260924_v8`; the uploaded v9 files contain the current 212-decision state. No profile or physical location was merged, no API request was made, and no regression BallTree or exposure code was changed.
